...
class CardGridNative2D;
class CardGridNative3D;

#include "card_instance.h"

class CardGridNative2D : public Control {
	GDCLASS(CardGridNative2D, Control);
...
private:
	...
	Vector<Control *> card_children;

public:
	GDVIRTUAL2R(Control *, _insert_card, int64_t, const Ref<CardInstance> &);
	GDVIRTUAL3(_update_card, int64_t, const Ref<CardInstance> &, Control *);
	GDVIRTUAL4(_reorder_card, int64_t, int64_t, const Ref<CardInstance> &, Control *);
	GDVIRTUAL3(_remove_card, int64_t, const Ref<CardInstance> &, Control *);
	...
	static CardGridNative2D *make();
};

class CardGridNative3D : public Node3D {
	GDCLASS(CardGridNative3D, Node3D);
