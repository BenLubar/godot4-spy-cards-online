...
Ref<T> get_predefined(E index)
{
	...
	ERR_FAIL_INDEX_V_MSG(index, predefined.size(), nullptr, String(PredefinedScriptKey<T>::name) + " outside of predefined range");

	Ref<T> def = predefined[index];
	ERR_FAIL_COND_V_MSG(def.is_null(), nullptr, String("predefined ") + PredefinedScriptKey<T>::name + " is null");
